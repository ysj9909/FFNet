...
def main():
    ...
    if args.eval:
        ...
        print(f"Accuracy of the network on {len(dataset_eval)} test images: {test_stats['top1']:.5f}%")
        test_stats = validate(
                model_ema.module, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix=" (EMA)",
            )
        print(f"Accuracy of the EMA network on {len(dataset_eval)} test images: {test_stats['top1']:.5f}%")
        return
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if output_dir is not None:
                update_summary(
                    epoch,
                    train_metrics,
                    eval_metrics if eval_metrics[eval_metric] > ema_eval_metrics[eval_metric] else ema_eval_metrics,
            ...
            if saver is not None:
                ...
                if eval_metrics[eval_metric] > ema_eval_metrics[eval_metric]:
