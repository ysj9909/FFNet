...
from models.FFNet_Layer import series_decomp, Flatten_Head
...
class Channel_Mixer(nn.Module):
    def __init__(self, large_size, small_size, dmodel, dff, nvars, small_kernel_merged = False, drop=0.1):
        ...
        self.dff = dff
        self.dw = RepVGGDW(in_channels=nvars * dmodel, out_channels=nvars * dmodel,
        ...
        self.ffnpw1 = nn.Conv1d(in_channels=nvars * dmodel, out_channels=nvars * dff, kernel_size=1, stride=1,
                                 padding=0, dilation=1, groups=nvars)
        self.act = nn.GELU()
        self.ffnpw2 = nn.Conv1d(in_channels=nvars * dff, out_channels=nvars * dmodel, kernel_size=1, stride=1,
                                 padding=0, dilation=1, groups= dmodel)
        self.ffn_drop1 = nn.Dropout(drop)
        self.ffn_drop2 = nn.Dropout(0.15)  #if drop >= 0.8 else nn.Dropout(0.3) # 0.15
    ...
    def forward(self,x):
        ...
        x = self.ffn_drop1(self.ffnpw1(x))
        x = self.act(x)  
        x = x.reshape(B, M, self.dff, N)
        x = x.permute(0, 2, 1, 3) 
        x = x.reshape(B, self.dff*M, N)
        x = self.ffn_drop2(self.ffnpw2(x)) 
        x = x.reshape(B, D, M, N)
        x = x.permute(0, 2, 1, 3)
...
class Token_Mixer(nn.Module):
    def __init__(self, large_size, small_size, dmodel, nvars, drop=0.1):

        super(Token_Mixer, self).__init__()
        self.er = 2   # we set ffn expansion ratio as 2 for all models
        dff = dmodel * self.er
        ...
        self.ffnpw2 = nn.Conv1d(in_channels=nvars * dff, out_channels=nvars * dmodel, kernel_size=1, stride=1,
                                 padding=0, dilation=1, groups=nvars)
        self.ffn_drop1 = nn.Dropout(0.3) #if self.er == 8 else nn.Dropout(0.3)  # 0.3
        self.ffn_drop2 = nn.Dropout(0.15) #if drop >= 0.8 else nn.Dropout(0.3)  # 0.15

        self.norm = nn.BatchNorm1d(dmodel)

        self.dw1 = nn.Conv1d(in_channels = nvars * dmodel, out_channels = nvars * dmodel, kernel_size = large_size,
                             stride = 1, padding = large_size // 2, groups = nvars * dmodel)
    
        self.act2 = nn.GELU()

        self.dw2 = nn.Conv1d(in_channels = nvars * dmodel, out_channels = nvars * dmodel, kernel_size = large_size,
                             stride = 1, padding = large_size // 2, groups = nvars * dmodel)
    ...
    def forward(self, x):
        ...
        x = self.ffn_drop1(self.ffnpw1(x))
        x = self.act1(x)
        x = x.reshape(B, D, self.er*M, N)
        x = x.permute(0, 2, 1, 3)
        x = x.reshape(B, -1, N)
        x = self.ffn_drop2(self.ffnpw2(x))   # B, M, D, N
...
class Block(nn.Module):
    def __init__(self, large_size, small_size, dmodel, dff, nvars, small_kernel_merged=False, drop=0.1):
        ...
        self.token_mixer = Token_Mixer(large_size = large_size, small_size = small_size, dmodel = dmodel, nvars = nvars)
        self.channel_mixer = Channel_Mixer(large_size = large_size, small_size = small_size, dmodel = dmodel, dff = dff, nvars = nvars, small_kernel_merged = small_kernel_merged, drop = drop)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.decomposition:
            ...
            self.model_res = FFNet(patch_size=self.patch_size,patch_stride=self.patch_stride,stem_ratio=self.stem_ratio, downsample_ratio=self.downsample_ratio, ffn_ratio=self.ffn_ratio, num_blocks=self.num_blocks, large_size=self.large_size, small_size=self.small_size, dims=self.dims, dw_dims=self.dw_dims,
            ...
            self.model_trend = FFNet(patch_size=self.patch_size,patch_stride=self.patch_stride,stem_ratio=self.stem_ratio, downsample_ratio=self.downsample_ratio, ffn_ratio=self.ffn_ratio, num_blocks=self.num_blocks, large_size=self.large_size, small_size=self.small_size, dims=self.dims, dw_dims=self.dw_dims,
                 nvars=self.nvars, small_kernel_merged=self.small_kernel_merged, backbone_dropout=self.drop_backbone, head_dropout=self.drop_head, use_multi_scale=self.use_multi_scale, revin=self.revin, affine=self.affine,
                 subtract_last=self.subtract_last, freq=self.freq, seq_len=self.seq_len, c_in=self.c_in, individual=self.individual, target_window=self.target_window)
        else:
            self.model = FFNet(patch_size=self.patch_size,patch_stride=self.patch_stride,stem_ratio=self.stem_ratio, downsample_ratio=self.downsample_ratio, ffn_ratio=self.ffn_ratio, num_blocks=self.num_blocks, large_size=self.large_size, small_size=self.small_size, dims=self.dims, dw_dims=self.dw_dims,
                 nvars=self.nvars, small_kernel_merged=self.small_kernel_merged, backbone_dropout=self.drop_backbone, head_dropout=self.drop_head, use_multi_scale=self.use_multi_scale, revin=self.revin, affine=self.affine,
                 subtract_last=self.subtract_last, freq=self.freq, seq_len=self.seq_len, c_in=self.c_in, individual=self.individual, target_window=self.target_window)
